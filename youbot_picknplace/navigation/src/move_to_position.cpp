...
MoveToPositionAction::MoveToPositionAction(ros::NodeHandle nh, std::string name) :
  ...
  as_(nh_, name, false),
  action_name_(name),
  nav_sub_(nh.subscribe("/" + std::string(std::getenv("ROBOT_NAME")) + "/odom", 1000, &MoveToPositionAction::currPositionCallback, this)),
  nav_pub_(nh.advertise<geometry_msgs::Twist>("/" + std::string(std::getenv("ROBOT_NAME")) + "/cmd_vel", 1000)) {
